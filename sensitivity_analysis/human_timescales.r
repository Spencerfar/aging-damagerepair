...
elsa <- read.csv('../datasets/human_data_pruned.csv', header = TRUE, sep = ",")
...
data <- read.csv('../datasets/human_repair_deficits_pruned.csv', header = TRUE, sep = ",")
...
write.csv(combined.fit, '../figure_data/figure5_supplement3/human_repair_timescale.csv')
...
wealth.repair <- ggplot() +
    ...
    ggtitle('g) ELSA humans (Phelps et al. 2020)') +
    geom_text(
        data    = pvals,
...
data <- read.csv('../datasets/human_damage_deficits_pruned.csv', header = TRUE, sep = ",")
data$wealth <- log(data$wealth + mean.wealth)
...
# save figure source data
write.csv(combined.fit, '../figure_data/figure5_supplement3/human_damage_timescale.csv')
...
ggsave("../plots/Human_wealth_timescale_pruned.pdf", img, , width=8, height=1.65)
